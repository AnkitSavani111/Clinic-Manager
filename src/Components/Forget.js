...
const Forget = () => {
  ...
  const handleOTP = async (e) => {
    ...
    try {
      sessionStorage.setItem("toResetPassEmail", email);
      ...
      if (resp.status === 200) {
        ...
        setEmailError("Please enter a valid email"); // Set error message for server-side error
      }
    } catch (error) {
      setEmailError("An error occurred. Please try again later."); // Set error message for other errors
    }
  };
